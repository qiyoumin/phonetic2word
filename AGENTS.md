...
- Before committing or pushing, verify the exact snapshot intended for the commit, not an uncommitted dirty working tree state
- Before final verification, run `git status --short`; if the same file appears in both staged and unstaged sections, clean up or restage so validation targets the intended snapshot
- Prefer validating after staging the intended files so local test results match the commit/CI snapshot as closely as possible
